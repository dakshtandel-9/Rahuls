...
function bodyFor(body, language) {
    if (body && typeof body === 'object') return body[language] ?? body.en;
    return body;
}

const ReviewCard = ({ img, name, username, body, language }) => {
    return (
        <figure
        ...
        >
            ...
            <blockquote className="mt-2 text-sm text-gray-800 dark:text-gray-300">
                &ldquo;{bodyFor(body, language)}&rdquo;
            </blockquote>
        </figure>
    );
};
                ...
                <div className="relative flex w-full flex-col items-center justify-center overflow-hidden">
                    ...
                    <Marquee pauseOnHover className="[--duration:25s]">
                        {firstRow.map((review, i) => (
                            <ReviewCard key={i} {...review} language={language} />
                        ))}
                    </Marquee>
                    {/* Row 2 — scrolls RIGHT */}
