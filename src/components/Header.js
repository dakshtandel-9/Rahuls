        ...
        >
            <div className="mx-auto max-w-7xl px-4">
                <div className="flex h-16 items-center justify-between">
                    {/* Brand */}
                    <a href="/" className="flex items-center gap-2" aria-label="Zocktech home">
                        <img
                    ...
                    <nav aria-label="Primary" className="hidden md:flex items-center gap-7">
                        <a href="/#services" className="nav-link" style={{ color: theme === 'light' ? '#555555' : undefined }}>{t('nav.services')}</a>
                        <a href="/#pricing" className="nav-link" style={{ color: theme === 'light' ? '#555555' : undefined }}>{t('nav.pricing')}</a>
                        <a href="/#portfolio" className="nav-link" style={{ color: theme === 'light' ? '#555555' : undefined }}>{t('nav.portfolio')}</a>
                        <a href="/#faqs" className="nav-link" style={{ color: theme === 'light' ? '#555555' : undefined }}>{t('nav.faqs')}</a>
                        <a href="/#contact" className="nav-link" style={{ color: theme === 'light' ? '#555555' : undefined }}>{t('nav.contact')}</a>
                    </nav>
...
function MobileMenu({ open, setOpen }) {
    ...
    return (
        <div
            id="mobile-menu"
            className={`md:hidden overflow-hidden transition-[max-height] duration-300 ease-out ${open ? 'max-h-[32rem]' : 'max-h-0'} border-t border-gray-300 dark:border-[#1f1f1f] bg-white dark:bg-[#0a0a0a]`}
            role="dialog"
            aria-modal="false"
        >
            <div className="mx-auto max-w-7xl px-4 py-4">
                ...
                <div className="mt-4 flex flex-col gap-3">
                    <div className="flex items-center justify-between gap-3">
                        <AnimatedThemeToggler className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-gray-200 dark:bg-[#1f1f1f] px-4 py-2.5 text-sm font-semibold text-black dark:text-white hover:bg-gray-300 dark:hover:bg-white/10" />
                        <LanguageSwitcher />
                    </div>
                    <ShimmerButton
                        asLink
